...
if ( (($_COOKIE['username']) != null)  && (($_COOKIE['password']) != null) ) {
    ...
    $res = mysqli_query($conn,"select * from t_user where name =  '$userName' ");
    $row = mysqli_fetch_assoc($res);
    if ($row['password'] == $password) {
        //验证通过后跳转到登录后的欢迎页面
        echo "<script>alert('登陆成功');location.href='index1.php';</script>";
    }
}
...
//第一次登陆的时候，通过用户输入的信息来确认用户
    ...
    $sql = "select name,password from t_user where name = '$userName' and password='$password'";
